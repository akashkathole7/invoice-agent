...
def grade_medium(
    ...
    score = 0.50 * field_accuracy + 0.40 * disc_f1 + 0.10 * efficiency
    return round(min(max(score, 0.0), 1.0), 4)
...
def grade_hard(
    ...
    """Grade Task 3 (Hard): Multi-document reconciliation.
    
    Score = 0.30 * field_accuracy + 0.40 * discrepancy_f1 + 0.30 * critical_flags
    """
    if not ground_truth_fields:
        return 0.0
    ...
    # --- Critical flags (30%) --- did agent catch the most important issues?
    ...
    score = 0.30 * field_accuracy + 0.40 * disc_f1 + 0.30 * critical_score - fp_penalty
    return round(min(max(score, 0.0), 1.0), 4)
